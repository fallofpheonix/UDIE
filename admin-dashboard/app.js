...
class UDIEDashboard {
    constructor() {
        ...
        this.baseUrl = window.location.origin.includes('localhost') ? 'http://localhost:3000' : '';
    ...
    async loadForecasts() {
        try {
            ...
            const res = await fetch(`${this.baseUrl}/forecast?h3_index=${h3.latLngToCell(center.lat, center.lng, 9)}`);
            const data = await res.json();
    ...
    async refreshHeatmap() {
        try {
            ...
            // City Dashboard returns heatmap summary and cells
            if (data.heatmapSummary) {
                document.querySelector('.stat-card .value.high').textContent = data.heatmapSummary.avgRisk.toFixed(2);
            }
            this.renderRiskGrid(data.heatmapSummary?.cells_data || []); // Assuming backend returns cell list in dashboard
        } catch (err) {
    ...
    async inspectCell(h3Index) {
        ...
        try {
            const res = await fetch(`${this.baseUrl}/intelligence/cell/${h3Index}`); // Updated Path
            const data = await res.json();

            inspector.querySelector('.h3-label').textContent = `Index: ${h3Index}`;
            inspector.querySelector('.incident-title').textContent = data.summary.explanation;
            inspector.querySelector('.fill').style.width = `${Math.min(100, data.summary.riskScore * 100)}%`; // Normalized 0-1
            inspector.querySelector('.val').textContent = `Reliability: ${data.summary.reliabilityScore.toFixed(2)}`;

            // Forecast Horizons (30m / 60m)
            const forecastText = `30m: ${(data.summary.forecastProbability * 100).toFixed(0)}% | 60m: --`;

            // If data contains actual horizons (from recent backend update)
            const h30 = data.details?.forecast?.forecast_30m;
            const h60 = data.details?.forecast?.forecast_60m;

            const updatedForecast = (h30 !== undefined)
                ? `30m Prob: ${(h30 * 100).toFixed(0)}% | 60m Prob: ${(h60 * 100).toFixed(0)}%`
                : forecastText;

            inspector.querySelector('.explanation p').innerHTML = `
                ${data.summary.explanation}<br>
    ...
    async loadHotspots() {
        try {
            const res = await fetch(`${this.baseUrl}/api/intelligence?type=HOTSPOT`);
            const data = await res.json();
    ...
    async loadSystemHealth() {
        try {
            const res = await fetch(`${this.baseUrl}/system-state`);
            const data = await res.json();
