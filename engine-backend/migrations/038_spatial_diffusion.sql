-- STEP 38: Spatial Diffusion (Law 5)
-- This implements the physical spread of risk intensity across the H3 grid.
...
-- STEP 39: Gradient Flow (Law 10)
-- Calculates the steepest risk gradient for each cell to identify disruption boundaries.

CREATE OR REPLACE FUNCTION calculate_risk_gradients()
RETURNS TABLE (
  h3_index BIGINT,
...
BEGIN
  RETURN QUERY
  WITH cell_neighbors AS (
    SELECT
      rc.h3_index AS source_h3,
      rc.weight AS source_weight,
      (neighbor.cell)::bigint AS neighbor_h3,
      COALESCE(nr.weight, 0.0) AS neighbor_weight
    FROM risk_cells rc
    CROSS JOIN LATERAL h3_grid_disk(rc.h3_index::h3index, 1) AS neighbor(cell)
    LEFT JOIN risk_cells nr ON nr.h3_index = neighbor.cell::bigint
    WHERE neighbor.cell::bigint != rc.h3_index
  ),
  diffs AS (
    SELECT
      source_h3,
      (source_weight - neighbor_weight) AS diff,
      neighbor_h3
    FROM cell_neighbors
  )
  SELECT DISTINCT ON (source_h3)
    source_h3,
    ABS(diff) AS gradient,
    neighbor_h3 AS direction_h3
  FROM diffs
  ORDER BY source_h3, ABS(diff) DESC;
END;
$$ LANGUAGE plpgsql;
