...
export class ForecastService {
  ...
  async rebuildForecastCells(): Promise<number> {
    ...
    const result = await this.db.query<QueryResultRow>(
      `WITH recent AS (
       ...
       INSERT INTO forecast_cells (h3_index, forecast_15m, forecast_30m, forecast_60m, source_points, updated_at)
       SELECT
         h3_index,
         GREATEST(0, s) AS forecast_15m,
       ...
       DO UPDATE SET
         forecast_15m = EXCLUDED.forecast_15m,
  ...
  async getForecast(h3Index: string) {
    ...
    return {
      ...
      forecast_15m: Number(row.forecast_15m),
