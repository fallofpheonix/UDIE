...
API_DASH_URL_PRIMARY="${API_DASH_URL_PRIMARY:-http://localhost:3000/api/v1/city-dashboard}"
API_DASH_URL_FALLBACK="${API_DASH_URL_FALLBACK:-http://localhost:3000/api/city-dashboard}"
...
if docker ps --format '{{.Names}}' | rg -q '^udie-postgres$' && \
   docker ps --format '{{.Names}}' | rg -q '^udie-redis$'; then
  log "Reusing existing udie-postgres/udie-redis containers"
else
