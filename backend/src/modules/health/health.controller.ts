    ...
    try {
      ...
      const stats = await this.databaseService.query<QueryResultRow>(
        `SELECT
           key,
           last_run,
           EXTRACT(EPOCH FROM (now() - last_run))::DOUBLE PRECISION AS lag_seconds
         FROM system_state
      ...
      const lockStats = await this.databaseService.query<QueryResultRow>(
        `SELECT 
          ...
          COALESCE(MAX(EXTRACT(EPOCH FROM (now() - wait_event_type))), 0)::DOUBLE PRECISION as max_wait_seconds
         FROM pg_stat_activity 
         WHERE wait_event_type = 'Lock'`
      );

      const maxWait = lockStats.rows[0]?.max_wait_seconds || 0;
      const status = (surfaceStale || laggingWorkers.length > 0 || replicaLag > 60 || maxWait > 10) ? 'degraded' : 'ok';
